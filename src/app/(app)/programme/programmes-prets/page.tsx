...
export default async function ProgrammesPretsPage({
  ...
  // Le fallback couvre les programmes qui n'ont qu'un seul visuel.
  const items = PROGRAMMES_PRETS.map((programme, index) => ({
    programme,
    photoUrl:
      (index % 2 === 0
        ? (programme.photoFemme ?? programme.photoHomme)
        : (programme.photoHomme ?? programme.photoFemme)) ?? null,
    gratuit: programme.slug === PROGRAMME_DECOUVERTE_GRATUIT_SLUG,
    deverrouille:
      programme.slug === PROGRAMME_DECOUVERTE_GRATUIT_SLUG ||
      accesComplet ||
      programmesAchetes.has(programme.slug),
  }));
