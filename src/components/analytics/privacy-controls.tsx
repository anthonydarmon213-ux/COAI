...
export function PrivacyControls() {
  ...
  return <>
    ...
    {open ? <section aria-label="Préférences de confidentialité" className={`${inline ? "relative mx-3 mb-6 sm:mx-auto" : "fixed inset-x-3 bottom-3 z-[100] mx-auto max-h-[65dvh] overflow-y-auto"} max-w-xl rounded-2xl border border-white/20 bg-[#101b23] p-3 text-white shadow-2xl sm:p-4`}>
      ...
      <p className="mt-2 text-xs text-slate-300">Choix conservé 6 mois, modifiable à tout moment via « Confidentialité » en bas de page.</p></>}
      <div className="mt-2 grid grid-cols-2 gap-2">
        <button type="button" onClick={() => choose(REFUSE_ALL)} className="min-h-11 rounded-xl border border-white/40 px-3 py-2 text-sm font-semibold">Tout refuser</button>
        <button type="button" onClick={() => choose({ audience: true, marketing: true })} className="min-h-11 rounded-xl border border-white/40 px-3 py-2 text-sm font-semibold">Tout accepter</button>
      </div>
      {details ? <button type="button" onClick={() => choose(draft)} className="mt-2 min-h-11 w-full rounded-xl bg-[#e0bd59] px-3 py-2 font-semibold text-black">Enregistrer mes choix</button> : <button type="button" onClick={() => setDetails(true)} className="min-h-11 px-3 text-xs underline">Personnaliser</button>}
      <a href="/confidentialite" className="inline-flex min-h-11 items-center px-3 text-xs underline">En savoir plus</a>
