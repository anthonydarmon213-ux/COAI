...
let writes=[], auth={id:'auth-A'}, user={id:'user-A'}, retrieved=0, saved=null;
...
let session;
const prisma={subscription:{findUnique:async()=>saved,create:async x=>{saved=x.data;writes.push({create:x.data});}},user:{findUnique:async()=>user,update:async x=>writes.push(x)}};
...
function reset(){writes=[];saved=null;retrieved=0;sub.status='trialing';auth={id:'auth-A'};user={id:'user-A'};session={status:'complete',mode:'subscription',client_reference_id:'user-A',subscription:sub};}
reset();auth=null;
...
reset();const confirmed=await POST(request({sessionId:'cs_test'}));assert.equal(confirmed.status,200);assert.equal(confirmed.body.accessActive,true);
...
for(const [input,expected] of [['active','ACTIVE'],['trialing','ACTIVE'],['past_due','PAST_DUE'],['unpaid','PAST_DUE'],['canceled','CANCELED'],['incomplete','INCOMPLETE'],['incomplete_expired','INCOMPLETE'],['paused','INCOMPLETE']]) {
 reset(); sub.status=input;
 assert.equal(sync.mapStripeStatus(input),expected);
 const result=await POST(request({sessionId:'cs_test'}));
 assert.equal(result.body.confirmed,true);
 assert.equal(result.body.accessActive,expected==='ACTIVE');
}
