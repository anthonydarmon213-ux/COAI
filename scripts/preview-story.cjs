...
const port = Number(process.env.COAI_STORY_PREVIEW_PORT || 3062);
...
const reposCode = ts.transpileModule(fs.readFileSync(path.join(root, 'src/lib/programmes/repos.ts'), 'utf8'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020 } }).outputText;
const data = {
  exercices: [
